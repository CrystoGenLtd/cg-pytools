...
class ShapeAnalyser:
    ...
    def analyse_crystal(
        ...
        if frame_idx is not None:
            ...
            self.frame_metrics[frame_idx] = self.shape_info(frame.coords)
        else:
            # Analyse all frames
...
# Usage example:
"""
# Load a crystal shape
crystal = CrystalShape.from_file("path/to/crystal.xyz")

# Create analyser
analyser = ShapeAnalyser()

# Analyse all frames
analyser.analyse_crystal(crystal)

# Or analyse specific frame
analyser.analyse_crystal(crystal, frame_idx=0)

# Get shape metrics for all frames
all_metrics = analyser.get_all_frame_metrics()
"""
