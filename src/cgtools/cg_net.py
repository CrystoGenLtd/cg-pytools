...
def main(input_file, output_file, group, group_key="r"):
    input_file = Path(input_file)
    output_file = Path(output_file)
    cg_net = CGNet(input_file)
    cg_net.parse()
    if group:
        cg_net.group_net(group_key)
    cg_net.write(output_file)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process and group net files.")
    parser.add_argument("-i", "--input_file", type=str, help="The input net file.")
    parser.add_argument("-o", "--output_file", type=str, help="The output net file.")
    parser.add_argument(
        "--group", action="store_true", help="Whether to group the net."
    )
    parser.add_argument(
        "-k",
        "--group-key",
        dest="group_key",
        default="r",
        choices=["r", "mol_type", "energy"],
        help="grouping type.",
    )

    args = parser.parse_args()
    main(args.input_file, args.output_file, args.group, args.group_key)
